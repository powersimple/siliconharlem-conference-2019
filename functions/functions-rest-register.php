...
add_action( 'rest_api_init', 'register_media_data' );
 function register_media_data() {
	...
	register_rest_field( 'attachment', 'data', array(//THE ROUTE IS MEDIA/the type is attachment
		'get_callback' => 'get_media'

		)
	);
}

function get_media( $object ) { //this function builds the data for a lean json packet of media
...
function get_post_tags($object){
	...
	add_action( 'rest_api_init', 'register_session_info' );
		
	function register_session_info() {
		
		register_rest_field( 'session', 'session_info', array(
			'get_callback' => 'get_session_info',
			'schema' => null,
			)
		);
	}
		
		function get_session_info( $object ) {
		$post_id = $object['id'];
			$session_info = array(
				"title"=>get_post_meta($post_id,"session_title",true),
				"url"=>get_post_meta($post_id,"session_url",true),
				//"client"=>get_post_meta($post_id,"session_client",true),
				//"agency"=>get_post_meta($post_id,"session_agency",true),
				"era"=>get_post_meta($post_id,"session_era",true)
			);
